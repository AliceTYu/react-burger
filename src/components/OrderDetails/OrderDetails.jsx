...
function OrderDetails() {
  ...
  return (
    <div className={`${styles.order} pt-15 pb-15 pr-25 pl-25`}>
      {!error && !loading &&
        <>
          <div className='text text_type_digits-large pb-8'>{order && order.order.number}</div>
      ...
      {loading && <div className='text text_type_main-medium pb-8'>Почти долетела до станции...</div>}
      {error && <div className='text text_type_main-medium pb-8'>Выберите булки или ингредиенты</div>}
    </div>
  )
}

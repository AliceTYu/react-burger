...
function LoginPage() {
    ...
    return (
        <form className={styles.login} onSubmit={loginEmailClick}>
            ...
            {errorLog && <div className={`${styles.error} text text_type_main-small pb-6`}>{errorLog}</div>}
            <Input type='email' placeholder='E-mail' extraClass='pb-6' onChange={onChangeEmail} value={emailLog} />
            <Input type='password' placeholder='Пароль' icon='ShowIcon' extraClass='pb-6' onChange={onChangePassword} value={passLog} />
            <Button htmlType='submit' type="primary" size="medium" extraClass="ml-2">
                Войти
            </Button>
            <div className={`text text_type_main-default text_color_inactive pt-20 pb-4`}>Вы — новый пользователь?
                <Link className={`${styles.link} pl-2`} to={'/register'}>Зарегистрироваться</Link>
            </div>
            <div className={`text text_type_main-default text_color_inactive`}>Забыли пароль?
